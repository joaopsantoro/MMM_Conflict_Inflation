...
*/
	...
	v[1]=V("Avg_Competitiveness");                  //sector average competitiveness
	v[2]=V("market_share_adjustment");              //market share adjustment paramter
	v[3]=V("Competitiveness");                      //firm's competitiveness
	if(v[1]!=0)                                     //if the sector average competitiveness is not zero
  	v[4]=v[0]+v[0]*v[2]*((v[3]/v[1])-1);          //firm's market share will be the last period's inscresed by the adjustment paramter times the ratio between firm's competitiveness and sector average competitiveness
...
*/
	...
	v[2]=V("Quality");                                                 //product quaility
...
EQUATION("Degree_Competition")
/*
Firm Variable
*/
	v[0]=V("Competitiveness");        //firm's competitiveness
	v[1]=V("Avg_Competitiveness");    //sector average competitiveness
	if(v[1]>0)                        //if sector average competitiveness is higher then one
		v[2]=v[0]/v[1];                 //firm's degree of competition is the firm's competitiveness over the sector's average
	else                              //if sector average competitiveness is not higher then one
		v[2]=0;                         //the degree is zero
RESULT(v[2])
